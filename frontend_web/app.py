...
st.set_page_config(
    ...
    menu_items={
        'Get Help': 'https://github.com/Dervoo/HealthSportMachineLearning',
        'About': "# Health-ML PWA\nSystem analityczny do monitorowania formy."
    }
)
...
# --- ML ENGINE ---
if "ml" not in st.session_state or "calculate_smart_goal" not in dir(st.session_state.ml):
    st.session_state.ml = MLEngine()
# Wymuś odświeżenie jeśli stara wersja nie miała training_log (naprawa błędu po aktualizacji)
import inspect
if "training_log" not in inspect.signature(st.session_state.ml.calculate_smart_goal).parameters:
    st.session_state.ml = MLEngine()
...
dynamic_smart = st.session_state.ml.calculate_smart_goal(
    st.session_state.ai_cfg["weight"], 180, 25, "Mężczyzna", 
    st.session_state.ai_cfg["activity"], st.session_state.ai_cfg.get("goal", "utrzymanie"),
    training_log=workout_str
)
st.session_state.target_kcal = dynamic_smart["target_kcal"]
st.session_state.protein_goal = dynamic_smart["target_p"]
...
m2.metric("Białko", f"{total_p:.1f}g / {st.session_state.protein_goal}g")
m3.metric("Nawodnienie", f"{total_hydration} L", f"Cel: {st.session_state.water_goal}L")
